...
@section('pageheading','Registered Vendors')
@section('cardheading','Add Registered Vendors')
...
<script>
    ...
    $(document).ready(function() {
        $('#user_table').DataTable({
            ...
            "order": [[ 0, "desc" ]],
            ajax:{
                url: "{{ route('admin.vendordet.index') }}",
            },
            columns:
            [
                {
        ...
        $('#sample_form').on('submit', function(event){
            ...
            if($('#action').val() == 'Add')
            {
                $.ajax
                ({
                    url:"{{ route('admin.vendordet.store') }}",
                    method:"POST",
            ...
            if($('#action').val() == "Update")
            {
                $.ajax
                ({
                    url:"{{ route('admin.vendordet.update') }}",
                    method:"POST",
                    data:new FormData(this),
                    contentType: false,
                    cache: false,
        ...
        $(document).on('click', '.delete', function(){
            ...
            {
                $.ajax({
                    url:"/admin/vendordet/destroy/"+id,
                    mehtod:"get",
                    data:{id:id},
                    success:function(data)
                    {
