...
@section('cardheading','Add Equipments Sub Theme')
@section('breadcrum','Home')
@section('subheading','Add Equipments Sub Theme')
...
<script>
    ...
    $(document).ready(function() {
        $('#user_table').DataTable({
            ...
            "order": [[ 0, "desc" ]],
            ajax:{
                url: "{{ route('admin.equipsubthemes.index') }}",
            },
            columns:
            [
                {
        ...
        $('#sample_form').on('submit', function(event){
            ...
            if($('#action').val() == 'Add')
            {
                $.ajax
                ({
                    url:"{{ route('admin.equipsubthemes.store') }}",
                    method:"POST",
            ...
            if($('#action').val() == "Update")
            {
                $.ajax
                ({
                    url:"{{ route('admin.equipsubthemes.update') }}",
                    method:"POST",
                    data:new FormData(this),
                    contentType: false,
                    cache: false,
        ...
        $(document).on('click', '.edit', function() {
            ...
            ({
                ...
                url:"/admin/equipsubthemes/"+id+"/edit",
                dataType:"json",
        ...
        $(document).on('click', '.delete', function(){
            ...
            {
                $.ajax({
                    url:"/admin/equipsubthemes/destroy/"+id,
                    mehtod:"get",
                    data:{id:id},
                    success:function(data)
                    {
        ...
        $(document).on('click', '.remove_subimages', function(){
            ...
            {
                $.ajax({
                    url:"/admin/equipsubthemes/delsubimage/"+id,
                    mehtod:"get",
                    data:{id:id},
                    success:function(data)
                    {
